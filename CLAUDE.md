...
## PM：pm-music（2026-09-23 建立）

在總機或任何其他目錄講「音藏」會召喚 `pm-music`。已經開在這個 repo 裡的對話不用召喚，主對話直接做，開工與收工照下面流程走。

- 定義檔真相源：總機 `_claude全域/agents/pm-music.md`；本 repo 的 `.claude/agents/pm-music.md` 是複本，給 claude.ai 網頁／手機的雲端 session 用。改一份就同步另一份
- PM 記憶放 `PM記憶/`（log 與工作偏好），跟著 git 走。**專案事實仍只寫 `00_現況.md`**
